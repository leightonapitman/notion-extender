...
const personalPageId: string = 'c4abc862c750443288e9726449f06bc0';
const dailyToDoIds: string[] = [
...
const dailyToDoCount: number = dailyToDoIds.length;
let completedDailyToDos: number = 0;
...
// Behaviors
function reportDailyToDos (dailyToDoIds: string[]) {
    const dailyToDoResponses: any = dailyToDoIds.map(toDoId => {
        return notion.blocks.retrieve({ block_id: toDoId });
    });

    Promise.all(dailyToDoResponses)
        .then((blockResults) => {
            blockResults.forEach(({ to_do: block }: any) => {
                if (block.checked) {
                    completedDailyToDos++;
                }
            });
...
function resetDailyToDo (toDoId: string, isChecked: boolean) {
...
function resetAllDailyToDos (dailyToDoIds: string[], isChecked: boolean) {
